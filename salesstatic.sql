...
(
  ...
  incomeYear        VARCHAR NOT NULL,
  income1           numeric DEFAULT 0,
  income2           numeric DEFAULT 0,
  income3           numeric DEFAULT 0,
  income4           numeric DEFAULT 0,
  income5           numeric DEFAULT 0,
  income6           numeric DEFAULT 0,
  income7           numeric DEFAULT 0,
  income8           numeric DEFAULT 0,
  income9           numeric DEFAULT 0,
  income10          numeric DEFAULT 0,
  income11          numeric DEFAULT 0,
  income12          numeric DEFAULT 0
);

COMMENT ON TABLE Income IS '실제입수금액';

COMMENT ON COLUMN Income.customerId IS '고객사코드';

COMMENT ON COLUMN Income.solutionName IS '솔루션명';

COMMENT ON COLUMN Income.contractStartDate IS '계약시작일';

COMMENT ON COLUMN Income.incomeYear IS '해당년도';

COMMENT ON COLUMN Income.income1 IS '1월입수금액';

COMMENT ON COLUMN Income.income2 IS '2월입수금액';

COMMENT ON COLUMN Income.income3 IS '3월입수금액';

COMMENT ON COLUMN Income.income4 IS '4월입수금액';

COMMENT ON COLUMN Income.income5 IS '5월입수금액';

COMMENT ON COLUMN Income.income6 IS '6월입수금액';

COMMENT ON COLUMN Income.income7 IS '7월입수금액';

COMMENT ON COLUMN Income.income8 IS '8월입수금액';

COMMENT ON COLUMN Income.income9 IS '9월입수금액';

COMMENT ON COLUMN Income.income10 IS '10월입수금액';

COMMENT ON COLUMN Income.income11 IS '11월입수금액';

COMMENT ON COLUMN Income.income12 IS '12월입수금액';
...
ALTER TABLE Income
  ADD CONSTRAINT FK_Contract_TO_Income
    FOREIGN KEY (customerId, solutionName, contractStartDate)
    REFERENCES Contract (customerId, solutionName, contractStartDate);
